...
- **Authenticated session** for `callitso.docker-uat01.ust.hk` in `./auth-profile` (Drupal + CAS/Microsoft login).
  Capture one with `pw-cli-agent import-session --capture --url https://callitso.docker-uat01.ust.hk/user/login` and complete the 2FA flow via noVNC (`http://localhost:6080/vnc.html`).
...
pw-cli-agent autorun --prompt-file ./prompts/prompts-uat-add-custom-mtpc-page.md \
